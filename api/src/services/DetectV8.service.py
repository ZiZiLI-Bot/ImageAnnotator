...
model = ort.InferenceSession(os.path.join(
    absolutePath, 'assets/models/YOLOv8_best.onnx'), providers=['CPUExecutionProvider'])
...
for result in results:
    ...
    draw.rectangle((x1, y1, x2, y2), None, "#00ff00", width=2)
    draw.text((x1, y1), labels[class_id]+":" +
              str(round(prob * 100, 1)) + "%", font=font, fill="#00ff00")

newName = f"{pathImage.split('.')[0]}_V8_OUT.png"
